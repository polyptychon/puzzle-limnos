...
describe('Store', () => {
  ...
  describe('Puzzle reducer', () => {
    ...
    it('should have keys: parts, emptyIndex, columns, rows, isPuzzleSolved', () => {
      expect(store.getState().puzzle).to.have.all.keys(
        'parts','emptyIndex','columns','rows', 'isPuzzleSolved'
      );
    });
  });
  describe('initAndShufflePuzzle', () => {
    ...
    it('should be initial state', () => {
      expect(store.getState().puzzle.parts).to.satisfy(
        parts => parts.every(({ index, },i) => index === i)
      );
    });
    it('should shuffle puzzle', () => {
      initAction.then(() => {
        expect(store.getState().puzzle.parts).to.not.satisfy(
          parts => parts.every(({ index, },i) => index === i)
        );
      });
    });
